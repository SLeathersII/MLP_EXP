...
class DxWMoonsNet(nn.Module):
    """ 
        d is number of layers
        w is width of hidden layers
    """
    def __init__(self, d:int = 3, w:int = 32, activation = F.relu, dtype = torch.float):
        """ 
            d is number of layers
            w is width of hidden layers
        """
        ...
        self.layers.append(nn.Linear(w,2, dtype=dtype))
        self.activation = activation
        
    def forward(self, x):
        for layer in self.layers:
            x = layer(x)
            if layer.out_features != 2: # use the set activation for all but output layer
                x = self.activation(x)
            else:    
