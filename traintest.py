...
def train_CEDA_gmm_out(model, device, train_loader, optimizer, epoch, 
                       lam=1., verbose=100, noise_loader=None, epsilon=.3,
                       dtype=torch.float, margin = np.log(4.)):
    criterion = nn.NLLLoss()
    model.train()
...
# use it out of scope to create points once on each call?
...
def plot_conf(net, data, axM, device, points = points):
    #plt.clf()
    ...
    cont = axM.contourf(x,y,z, vmin=.5, vmax=1., extend='neither')
    cax = axM.inset_axes([1,0,0.04,1])
    cb = plt.colorbar(cont, ax=axM, cax=cax, orientation='vertical')
    axM.scatter(data[:, 0].cpu(), data[:, 1].cpu(), s=3, color=colors[output_data.max(1)[1]])
    plt.xlim([0, 1])
    plt.ylim([0, 1])
    plt.xticks([])
    plt.yticks([])
